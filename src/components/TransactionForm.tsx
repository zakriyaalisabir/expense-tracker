...
export default function TransactionForm(){
  ...
  const [form,setForm] = React.useState<any>({
    ...
    category_id: categories.find(c=>c.type==="expense")?.id ?? "", description: "", tags: ""
  });
  React.useEffect(()=>{
      ...
      <DialogContent>
        <Stack spacing={2} sx={{ mt: 1 }}>
          ...
          <TextField select label="Category" value={form.category_id} onChange={e=>setForm({...form, category_id:e.target.value})}>
            {useAppStore.getState().categories.filter(c=>c.type===type).map(c=>(<MenuItem key={c.id} value={c.id}>{c.name}</MenuItem>))}
          </TextField>
